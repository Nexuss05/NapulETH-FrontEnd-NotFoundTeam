...
const season1TaskIcons: Record<string, JSX.Element> = {
  '1': <Wallet className="w-20 h-20 text-cyber-blue" />,
  '2': <Shield className="w-20 h-20 text-cyber-purple" />,
  '3': <Activity className="w-20 h-20 text-cyber-teal" />,
  '4': <Coins className="w-20 h-20 text-cyber-pink" />,
  '5': <Target className="w-20 h-20 text-cyber-green" />,
};
...
const Index = () => {
  ...
  const [hasDisconnected, setHasDisconnected] = useState(false); // Track if user manually disconnected
  ...
  useEffect(() => {
    ...
    } else if (!wagmiIsConnected && isConnected) {
  ...
  }, [wagmiIsConnected, wagmiAddress, isConnected, walletAddress, hasDisconnected]);
  ...
  const handleWalletConnect = (address: string) => {
    ...
    setIsConnected(true);
    setHasDisconnected(false); // Reset disconnection state
  ...
  const confirmDisconnect = async () => {
    try {
      // Disconnect wagmi connection first
      await wagmiDisconnect();
      
      // Then update local state
      setIsConnected(false);
      setWalletAddress('');
      setHasDisconnected(true); // Mark as manually disconnected
      setShowLogoutDialog(false);
    } catch (error) {
      console.error('Failed to disconnect:', error);
      // Update local state even if wagmi disconnect fails
      setIsConnected(false);
      setWalletAddress('');
  ...
  if (!isConnected) {
    return <RealWalletConnect onConnect={handleWalletConnect} isReconnecting={hasDisconnected} />;
  }
          ...
          <div className="space-y-4">
            <div>
              <p className="text-sm text-muted-foreground">
                Are you sure you want to disconnect your wallet? Your progress will be saved and you can reconnect anytime.
              </p>
            </div>
            <div className="p-3 rounded-lg bg-amber-500/10 border-amber-500/20">
             <div className="flex items-center gap-2">
               <AlertCircle className="w-4 h-4 text-amber-500" />
               <span className="text-sm font-medium text-amber-500">Don't worry!</span>
             </div>
             <p className="text-sm text-muted-foreground mt-1">
               Your XP ({currentXP}), completed tasks, and progress are automatically saved.
             </p>
           </div>
